...
'    SUM(IIF(EXTRACT(MONTH FROM mov.MOV_DATA) = 1, mov.MOV_VALORFINAL - COALESCE((SELECT SUM(mi.MITENS_QUANTIDADE * prod.PRO_CUSTO) '+
'FROM MOVIMENTOITENS mi JOIN PRODUTO prod ON prod.PRO_CODIGO = mi.MITENS_PRODUTO WHERE mi.MITENS_MOVIMENTO = mov.MOV_CODIGO), 0), 0)) AS Janeiro,  '+
'    SUM(IIF(EXTRACT(MONTH FROM mov.MOV_DATA) = 2, mov.MOV_VALORFINAL - COALESCE((SELECT SUM(mi.MITENS_QUANTIDADE * prod.PRO_CUSTO) '+
'FROM MOVIMENTOITENS mi JOIN PRODUTO prod ON prod.PRO_CODIGO = mi.MITENS_PRODUTO WHERE mi.MITENS_MOVIMENTO = mov.MOV_CODIGO), 0), 0)) AS Fevereiro, '+
'    SUM(IIF(EXTRACT(MONTH FROM mov.MOV_DATA) = 3, mov.MOV_VALORFINAL - COALESCE((SELECT SUM(mi.MITENS_QUANTIDADE * prod.PRO_CUSTO)  '+
'FROM MOVIMENTOITENS mi JOIN PRODUTO prod ON prod.PRO_CODIGO = mi.MITENS_PRODUTO WHERE mi.MITENS_MOVIMENTO = mov.MOV_CODIGO), 0), 0)) AS Marco,  '+
'   SUM(IIF(EXTRACT(MONTH FROM mov.MOV_DATA) = 4, mov.MOV_VALORFINAL - COALESCE((SELECT SUM(mi.MITENS_QUANTIDADE * prod.PRO_CUSTO)    '+
'FROM MOVIMENTOITENS mi JOIN PRODUTO prod ON prod.PRO_CODIGO = mi.MITENS_PRODUTO WHERE mi.MITENS_MOVIMENTO = mov.MOV_CODIGO), 0), 0)) AS Abril,    '+
'   SUM(IIF(EXTRACT(MONTH FROM mov.MOV_DATA) = 5, mov.MOV_VALORFINAL - COALESCE((SELECT SUM(mi.MITENS_QUANTIDADE * prod.PRO_CUSTO)     '+
'FROM MOVIMENTOITENS mi JOIN PRODUTO prod ON prod.PRO_CODIGO = mi.MITENS_PRODUTO WHERE mi.MITENS_MOVIMENTO = mov.MOV_CODIGO), 0), 0)) AS Maio,     '+
'   SUM(IIF(EXTRACT(MONTH FROM mov.MOV_DATA) = 6, mov.MOV_VALORFINAL - COALESCE((SELECT SUM(mi.MITENS_QUANTIDADE * prod.PRO_CUSTO)      '+
'FROM MOVIMENTOITENS mi JOIN PRODUTO prod ON prod.PRO_CODIGO = mi.MITENS_PRODUTO WHERE mi.MITENS_MOVIMENTO = mov.MOV_CODIGO), 0), 0)) AS Junho,    '+
'   SUM(IIF(EXTRACT(MONTH FROM mov.MOV_DATA) = 7, mov.MOV_VALORFINAL - COALESCE((SELECT SUM(mi.MITENS_QUANTIDADE * prod.PRO_CUSTO)       '+
'FROM MOVIMENTOITENS mi JOIN PRODUTO prod ON prod.PRO_CODIGO = mi.MITENS_PRODUTO WHERE mi.MITENS_MOVIMENTO = mov.MOV_CODIGO), 0), 0)) AS Julho,    '+
'   SUM(IIF(EXTRACT(MONTH FROM mov.MOV_DATA) = 8, mov.MOV_VALORFINAL - COALESCE((SELECT SUM(mi.MITENS_QUANTIDADE * prod.PRO_CUSTO)        '+
'FROM MOVIMENTOITENS mi JOIN PRODUTO prod ON prod.PRO_CODIGO = mi.MITENS_PRODUTO WHERE mi.MITENS_MOVIMENTO = mov.MOV_CODIGO), 0), 0)) AS Agosto,     '+
'   SUM(IIF(EXTRACT(MONTH FROM mov.MOV_DATA) = 9, mov.MOV_VALORFINAL - COALESCE((SELECT SUM(mi.MITENS_QUANTIDADE * prod.PRO_CUSTO)         '+
'FROM MOVIMENTOITENS mi JOIN PRODUTO prod ON prod.PRO_CODIGO = mi.MITENS_PRODUTO WHERE mi.MITENS_MOVIMENTO = mov.MOV_CODIGO), 0), 0)) AS Setembro,     '+
'   SUM(IIF(EXTRACT(MONTH FROM mov.MOV_DATA) = 10, mov.MOV_VALORFINAL - COALESCE((SELECT SUM(mi.MITENS_QUANTIDADE * prod.PRO_CUSTO)         '+
'FROM MOVIMENTOITENS mi JOIN PRODUTO prod ON prod.PRO_CODIGO = mi.MITENS_PRODUTO WHERE mi.MITENS_MOVIMENTO = mov.MOV_CODIGO), 0), 0)) AS Outubro,    '+
'   SUM(IIF(EXTRACT(MONTH FROM mov.MOV_DATA) = 11, mov.MOV_VALORFINAL - COALESCE((SELECT SUM(mi.MITENS_QUANTIDADE * prod.PRO_CUSTO)          '+
'FROM MOVIMENTOITENS mi JOIN PRODUTO prod ON prod.PRO_CODIGO = mi.MITENS_PRODUTO WHERE mi.MITENS_MOVIMENTO = mov.MOV_CODIGO), 0), 0)) AS Novembro,    '+
'    SUM(IIF(EXTRACT(MONTH FROM mov.MOV_DATA) = 12, mov.MOV_VALORFINAL - COALESCE((SELECT SUM(mi.MITENS_QUANTIDADE * prod.PRO_CUSTO)          '+
'FROM MOVIMENTOITENS mi JOIN PRODUTO prod ON prod.PRO_CODIGO = mi.MITENS_PRODUTO WHERE mi.MITENS_MOVIMENTO = mov.MOV_CODIGO), 0), 0)) AS Dezembro     '+
'FROM             '+
'    MOVIMENTO mov  '+
'WHERE               '+
'    EXTRACT(YEAR FROM mov.MOV_DATA) = 2023 ');
