...
begin
  ...
  begin
    ...
    begin
      ...
      Movimento.ObsPagamento       := FieldByName('Mov_ObsPagamento').AsString;
    end;
    result := Movimento;
  end
  else
  ...
  try
    if FStatus in [0,1] then
      QryCadastrar.sql.Add('INSERT INTO MOVIMENTO '+
                            ...
                            'MOV_FORPAGTO,Mov_Condicoes,MOV_Entrega,Mov_ObsPagamento)'+
                            'VALUES '+
                            '(:MOV_CODIGO,:MOV_CLIENTE,:MOV_DATA,:MOV_TIPO, '+
                            ':MOV_TOTALPRODUTOS,:MOV_TOTALSERVICOS, '+
                            ':MOV_ACRESCIMO,:MOV_DESCONTO,:MOV_FRETE, '+
    ...
    if FStatus = 2 then
      QryCadastrar.sql.Add('UPDATE MOVIMENTO SET  '+
                           ...
                           'MOV_Entrega=:MOV_Entrega,Mov_ObsPagamento= ' +
                           ':Mov_ObsPagamento, where MOV_CODIGO = '+Movimento.Codigo);
    ...
    begin
      ...
      params[15].AsString := Movimento.ObsPagamento;
      ExecSql;
    end;
