...
begin
  ...
  begin
    ...
    begin
      With QryPes do
      begin
        ForPagto := FormaPagtoDAO.Encontrar(FieldByName('pag_formapagto').AsString);

        Pagamento.Codigo := FieldByName('pag_codigo').AsString;
        Pagamento.DataVcto:= FieldByName('PAG_DATAVCTO').AsDateTime;
        Pagamento.FormaPagto := ForPagto.Clone(ForPagto);
        Pagamento.Valor := FieldByName('pag_valor').AsFloat;
        Pagamento.Movimento := FieldByName('pag_Movimento').AsString;

        Movimento.Descricao.Add(Pagamento.Clone(Pagamento));
      end;
      QryPes.Next
    end;
...
begin
  for Descricao in MovimentoASalvar.Descricao do
    Salvar(Descricao);
end;
