...
uses ProdutoDTO,FornecedorDTO,FornecedorDAO,FireDAC.Stan.Intf, FireDAC.Stan.Option,
    ...
    FireDAC.DApt.Intf, FireDAC.Stan.Async, FireDAC.DApt, Data.DB,CategoriaDAO,
...
begin
  ...
  begin
    ...
    begin
      ...
      Produto.Codigo := FieldByName('PRO_CODIGO').AsString;
      Produto.Fornecedor := FornecedorDTO;
      Produto.Nome := FieldByName('PRO_NOME').AsString;
      Produto.Custo := FieldByName('PRO_CUSTO').AsFloat;
      Produto.ValorVenda := FieldByName('pro_valorvenda').AsFloat;
      Produto.Descricao := FieldByName('pro_descricao').AsString;
      Produto.Medidas := FieldByName('pro_medidas').AsString;
      Produto.Tecido := FieldByName('pro_tecido').AsString;
      Produto.Madeira := FieldByName('pro_madeira').AsString;
      Produto.Categoria := CategoriaDAO.Encontrar(FieldByName('pro_categoria').AsString);
      result := Produto;
    end;
...
begin
  ...
  QryCadastro.ParamByName('pro_Categoria').AsString := ProdutoDTO.Categoria.Codigo;
